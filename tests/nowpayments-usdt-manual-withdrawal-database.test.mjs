...
test("simplified migration exposes only atomic service-role completion and rejection", async (t) => {
  ...
  const exactAclRows = (await db.query(`
    select
      p.proname,
      coalesce((
        select jsonb_agg(
    ...
    order by p.proname
  `)).rows;
  assert.equal(exactAclRows.length, 15);
  for (const row of exactAclRows) {
    const serviceCallable = new Set([
      "request_nowpayments_usdt_withdrawal",
      "complete_nowpayments_usdt_withdrawal_manual",
      "reject_nowpayments_usdt_withdrawal_manual",
    ]).has(row.proname);
...
async function simplifiedPreflightFingerprint(client) {
  return (await client.query(`
    ...
    select jsonb_build_object(
      ...
      'config', (
        select jsonb_agg(to_jsonb(config_row) order by config_row.id)
        from public.nowpayments_usdt_config config_row
      ),
      'wallets', (
        select jsonb_agg(to_jsonb(wallet_row) order by wallet_row.user_id)
        from public.nowpayments_usdt_wallets wallet_row
      ),
      'withdrawals', (
        select coalesce(jsonb_agg(to_jsonb(withdrawal_row) order by withdrawal_row.id), '[]'::jsonb)
      ...
      'broadcasts', (
        select coalesce(jsonb_agg(to_jsonb(broadcast_row) order by broadcast_row.id), '[]'::jsonb)
        from public.nowpayments_usdt_withdrawal_broadcasts broadcast_row
      ),
      'verifications', (
        select coalesce(jsonb_agg(to_jsonb(verification_row) order by verification_row.id), '[]'::jsonb)
        from public.nowpayments_usdt_withdrawal_verifications verification_row
      ),
      'ledger', (
        select coalesce(jsonb_agg(to_jsonb(ledger_row) order by ledger_row.id), '[]'::jsonb)
...
test("native PostgreSQL serializes requests, admin actions, and balance races on profile rows", {
  ...
  await t.test("simplified migration rejects complete catalog drift before any replacement or row mutation", async (st) => {
    const fixtures = [
      {
        name: "authenticated EXECUTE on a retired function",
        drift: `
          grant execute on function
            public.complete_nowpayments_usdt_withdrawal(
              uuid,uuid,text,text,integer,text,boolean,boolean,text,text,
      ...
      {
        name: "PUBLIC EXECUTE on a retired function",
        drift: `
          grant execute on function
            public.reject_nowpayments_usdt_withdrawal(uuid,uuid,text,text)
          to public
      ...
      {
        name: "service_role EXECUTE WITH GRANT OPTION",
        drift: `
          grant execute on function
            public.lock_nowpayments_usdt_withdrawal_send(
      ...
      {
        name: "unexpected function grantee and grantor",
        drift: `
          do $role$
          begin
            if to_regrole('unexpected_catalog_grantor') is null then
              create role unexpected_catalog_grantor;
            end if;
          end
          $role$;
          grant execute on function
            public.record_nowpayments_usdt_withdrawal_broadcast(
              uuid,uuid,text,text,text
            )
          to unexpected_catalog_grantor with grant option;
          grant usage on schema public to unexpected_catalog_grantor;
          set role unexpected_catalog_grantor;
          grant execute on function
            public.record_nowpayments_usdt_withdrawal_broadcast(
              uuid,uuid,text,text,text
            )
      ...
      {
        name: "unexpected inherited function owner",
        drift: `
          do $role$
          begin
            if to_regrole('unexpected_catalog_owner') is null then
              create role unexpected_catalog_owner;
            end if;
          end
          $role$;
          alter function
          ...
          owner to unexpected_catalog_owner
        `,
        error: /function catalog/,
      },
      {
        name: "inherited function source drift",
        drift: `
          create or replace function public.claim_nowpayments_usdt_withdrawal_review(
          ...
          as $function$
          begin
            return jsonb_build_object('status', 'catalog_drift');
          end;
          $function$
        `,
        error: /function catalog/,
      ...
      {
        name: "inherited function security drift",
        drift: `
          alter function
            public.claim_nowpayments_usdt_withdrawal_review(uuid,uuid,text)
          security invoker
      ...
      {
        name: "inherited function return drift",
        drift: `
          alter function
            public.claim_nowpayments_usdt_withdrawal_review(uuid,uuid,text)
          rename to claim_nowpayments_usdt_withdrawal_review_catalog_drift;
      ...
      {
        name: "destination validator source drift",
        drift: `
          create or replace function public.assert_safe_nowpayments_usdt_withdrawal_destination(
            p_value text
          )
          returns text
          language plpgsql
      ...
      {
        name: "destination validator owner drift",
        drift: `
          do $role$
          begin
            if to_regrole('unexpected_validator_owner') is null then
              create role unexpected_validator_owner;
            end if;
          end
          $role$;
          alter function
            public.assert_safe_nowpayments_usdt_withdrawal_destination(text)
          owner to unexpected_validator_owner
        `,
        error: /function catalog/,
      },
      {
        name: "destination validator security drift",
        drift: `
          alter function
            public.assert_safe_nowpayments_usdt_withdrawal_destination(text)
          security invoker
      ...
      {
        name: "destination validator ACL drift",
        drift: `
          grant execute on function
            public.assert_safe_nowpayments_usdt_withdrawal_destination(text)
          to authenticated
      ...
      {
        name: "disabled withdrawal immutability trigger",
        drift: `
          alter table public.nowpayments_usdt_withdrawals
          disable trigger enforce_nowpayments_usdt_withdrawal_immutability
        `,
        error: /trigger catalog/,
      },
      {
        name: "wrong withdrawal immutability trigger timing",
        drift: `
          drop trigger enforce_nowpayments_usdt_withdrawal_immutability
            on public.nowpayments_usdt_withdrawals;
          create trigger enforce_nowpayments_usdt_withdrawal_immutability
          after update on public.nowpayments_usdt_withdrawals
      ...
      {
        name: "wrong withdrawal immutability trigger function",
        drift: `
          drop trigger enforce_nowpayments_usdt_withdrawal_immutability
            on public.nowpayments_usdt_withdrawals;
          create trigger enforce_nowpayments_usdt_withdrawal_immutability
          ...
          for each row execute function
            public.set_nowpayments_usdt_updated_at()
        `,
        error: /trigger catalog/,
      },
      {
        name: "altered withdrawal constraint",
        drift: `
          alter table public.nowpayments_usdt_withdrawals
            drop constraint nowpayments_usdt_withdrawals_minimum_check,
            add constraint nowpayments_usdt_withdrawals_minimum_check
              check (
                gross_amount_usdt >= 2
                and destination_address is not null
              )
        `,
        error: /constraint catalog/,
      },
      {
        name: "unvalidated withdrawal constraint",
        drift: `
          alter table public.nowpayments_usdt_withdrawals
            drop constraint nowpayments_usdt_withdrawals_destination_check,
            add constraint nowpayments_usdt_withdrawals_destination_check
              check (destination_address ~ '^0x[0-9a-f]{40}$') not valid
        `,
        error: /constraint catalog/,
      },
      {
        name: "withdrawal RLS disabled",
        drift: `
          alter table public.nowpayments_usdt_withdrawals
          disable row level security
        `,
        error: /relation, RLS, policy, or grant catalog/,
      },
      {
        name: "unexpected withdrawal policy",
        drift: `
          create policy unexpected_catalog_policy
          on public.nowpayments_usdt_withdrawals
      ...
      {
        name: "unexpected table grant",
        drift: `
          grant insert on table public.nowpayments_usdt_withdrawals
          to authenticated
        `,
        error: /relation, RLS, policy, or grant catalog/,
      },
      {
        name: "invalid relevant index",
        drift: `
          update pg_catalog.pg_index
          set indisvalid = false
          where indexrelid =
            'public.idx_nowpayments_usdt_withdrawals_status'::regclass
        `,
        error: /index or sequence catalog/,
      },
      {
        name: "unready relevant index",
        drift: `
          update pg_catalog.pg_index
      ...
      {
        name: "altered relevant index definition",
        drift: `
          drop index public.idx_nowpayments_usdt_withdrawals_status;
          create index idx_nowpayments_usdt_withdrawals_status
          on public.nowpayments_usdt_withdrawals(created_at, status)
        `,
        error: /index or sequence catalog/,
      },
      {
        name: "altered relevant index predicate",
        drift: `
          drop index public.nowpayments_usdt_withdrawals_one_open_per_user;
          create unique index nowpayments_usdt_withdrawals_one_open_per_user
          on public.nowpayments_usdt_withdrawals(user_id)
          where status in ('reserved', 'reviewing')
        `,
        error: /index or sequence catalog/,
      },
    ];
    ...
    for (const fixture of fixtures) {
      await st.test(fixture.name, async () => {
        await resetNative();
        ...
        const before = await simplifiedPreflightFingerprint(observer);
        assert.deepEqual(before.replacement_functions, []);
        await assert.rejects(
          applyMigration(nativeDb(observer), simplifiedWithdrawalMigration),
          fixture.error,
        );
        const after = await simplifiedPreflightFingerprint(observer);
        assert.deepEqual(after, before);
        assert.deepEqual(after.replacement_functions, []);
      });
    }
  });
