...
type PublicWithdrawalStatus = "pending" | "completed" | "rejected";
...
function publicStatus(status: string): PublicWithdrawalStatus {
...
export default async (req: Request, context?: Context): Promise<Response> => {
  ...
  const [walletResult, withdrawalsResult] = await Promise.all([
    ...
    admin
      ...
      .select(
        "id,user_id,destination_address,gross_amount_usdt::text,fee_percent::text,fee_amount_usdt::text,net_amount_usdt::text,status,requested_at",
      )
      .eq("user_id", userId)
