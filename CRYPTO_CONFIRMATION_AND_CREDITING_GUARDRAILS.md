...
- each row's stored `tx_hash`, `event_index`, `from_address`, `to_address`, `amount_raw`, and `block_number`
- the BSC USDT contract address used by the detection helper
- the canonical transaction receipt or canonical log returned by the configured BSC RPC provider
...
### Canonical log revalidation

Before a detected deposit can be marked `confirmed`, the confirmation helper must re-query the canonical BSC chain and verify that the original ERC-20 `Transfer` log still exists on the canonical chain.

The helper must not rely only on the stored `block_number` plus latest block height. A detected row may have been inserted from a block that is later reorged out. If confirmation only counts block height, a non-canonical or removed transfer could incorrectly become eligible for crediting.

For each candidate BSC deposit, the confirmation helper must fetch the canonical transaction receipt or canonical log by `tx_hash` and verify all of the following before setting `confirmed_at` or making the row eligible for future crediting:

- the transaction receipt exists
- the receipt/log is on the canonical chain according to the RPC provider
- the log at the stored `event_index` or `logIndex` exists
- the log contract address is the BSC USDT contract
- topic0 is the ERC-20 `Transfer(address,address,uint256)` event topic
- the recipient address matches `crypto_deposits.to_address`
- the sender address matches `crypto_deposits.from_address` when present
- the raw token amount matches `crypto_deposits.amount_raw` exactly
- the canonical block number exists and is consistent with the stored row, or any mismatch is treated as skipped/manual-review
- removed, reorged, missing, or mismatched logs are never marked confirmed

If the canonical receipt/log cannot be verified, the helper must skip the row and report it to the admin. It must not mark the row confirmed and must not credit it.

A future schema hardening PR may add `block_hash` or a canonical-log verification timestamp, but confirmation must still revalidate the canonical receipt/log before any money-moving phase depends on the row.

### Confirmation calculation

For a detected deposit with a valid `block_number` and verified canonical log:

```text
confirmations = max(0, latest_block_number - canonical_block_number + 1)
```
...
- `confirmations`
- `status`, from `detected` to `confirmed` only after the threshold is reached and canonical log revalidation succeeds
...
Rows whose canonical receipt/log cannot be verified must remain unconfirmed and should be surfaced for manual review.
...
- rows scanned
- rows with canonical logs verified
- rows updated
- rows newly marked confirmed
- rows skipped because already credited, swept, failed, malformed, or missing block numbers
- rows skipped because receipt/log is missing
- rows skipped because receipt/log does not match the stored deposit
- rows skipped because the log is removed, reorged, or not canonical
...
3. Confirmation behavior must be implemented with canonical log revalidation, or deliberately documented as manual-only for the first crediting test.
...
- recipient address
- amount USDT
- canonical receipt/log verification status
- confirmation count
...
- Was the canonical receipt/log revalidated before confirmation?
- Was the row skipped because the receipt/log was missing, mismatched, or reorged?
...
2. Add a BSC confirmation design or admin-only confirmation dry-run helper that revalidates canonical receipts/logs.
3. Add an admin-only BSC confirmation write helper that updates only confirmation fields and status `detected -> confirmed` after canonical log revalidation succeeds.
4. Add an admin audit view update showing confirmation transition and canonical-log verification details.
