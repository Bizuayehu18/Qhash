-- ==========================================================
-- Align approve_deposit_tx source with live-repaired RPC
--
-- Context: The live Supabase RPC was manually repaired to:
--   1. Require p_amount > 0 for approve (no deposits.amount fallback)
--   2. Remove the reviewed_by column reference (column does not exist)
--
-- Prior migrations in this directory are OUTDATED:
--   - 20260518000000: still had COALESCE fallback + reviewed_by
--   - 20260519000000: removed COALESCE but still had reviewed_by
--
-- This migration is the canonical source-of-truth representation
-- of the CURRENT live approve_deposit_tx function.
--
-- See: DEPOSIT_SAFETY_CHECKPOINT.md for the authoritative rules.
-- ==========================================================
...
BEGIN
  ...
  --    NOTE: No reviewed_by column — that column does not exist on the deposits table.
