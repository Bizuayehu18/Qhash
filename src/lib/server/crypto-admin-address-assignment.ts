...
function normalizeAddress(network: CryptoNetwork, value: unknown): string {
  ...
  if (network === "TRON") {
    if (!/^T[1-9A-HJ-NP-Za-km-z]{33}$/.test(address)) {
      throwSafe("ADMIN", "Invalid TRON address.", "TRON address must be a 34 character base58 address starting with T");
    }
  ...
  if (!/^0x[a-fA-F0-9]{40}$/.test(address)) {
    throwSafe("ADMIN", "Invalid BSC address.", "BSC address must be a 0x-prefixed 40 byte hex address");
  }
