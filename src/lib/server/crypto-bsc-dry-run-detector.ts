...
async function scanAssignedBscTransfers(
  ...
  for (const recipientTopics of recipientTopicBatches) {
    ...
    for (const log of logs) {
      ...
      const contractAddress = normalizeBscAddress(item.address);
      const eventTopic = toStringOrNull(topics?.[0])?.toLowerCase() ?? null;
      ...
      if (
        contractAddress !== BSC_USDT_CONTRACT_ADDRESS ||
