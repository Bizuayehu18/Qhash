...
- prevent newly stored detected rows from becoming visible to normal users while crypto deposits are still disabled
...
## User visibility boundary for detected rows

Before any future PR inserts detected rows into `crypto_deposits`, it must explicitly handle normal-user visibility for those rows.

Current user-facing crypto deposit behavior already gates assigned address exposure behind `crypto_auto_credit_enabled`, but existing user deposit-history loading must also be reviewed before persistent detection storage. If a storage PR inserts `status = 'detected'` rows while `crypto_auto_credit_enabled` remains false, those rows must not appear to normal users as live deposit history unless that user-facing visibility is separately scoped and reviewed.

A future storage PR must choose one of these safe paths before inserting detected rows:

1. Gate user-visible crypto deposit history behind `crypto_auto_credit_enabled`, similar to assigned address exposure.
2. Store early detected rows in an admin-only staging/audit path instead of returning them through the normal user crypto overview.
3. Add a separately reviewed visibility rule so `status = 'detected'` audit rows remain hidden from normal users until user-facing crypto deposits are intentionally enabled.

The server must enforce the visibility boundary. The UI must not be the only protection.

Until this is implemented and reviewed, persistent detection storage remains admin/audit-only design work and must not be treated as a live user deposit feature.
...
- normal users cannot see newly inserted detected audit rows while crypto deposits remain disabled
...
1. Review and implement user-visible crypto deposit-history gating or an admin-only staging path before persistent detected rows are inserted.
2. Implement admin-only BSC detected-row storage, status `detected`, no crediting, no watcher-state update.
3. Add admin-only detected-deposit audit view.
4. Add confirmation policy and `detected` to `confirmed` transition.
5. Add manual review/approval crediting design.
6. Implement crediting only after separate review.
7. Consider user-facing address exposure only after detection, confirmation, audit, and crediting are reviewed.
8. Design sweeping/signing separately.

## Current conclusion

The next code step may store BSC detections as pending/detected audit rows only after the user-visibility boundary is handled. It must not credit wallets, update balances, expose addresses to users, sweep, sign, generate addresses, or enable crypto deposits.
