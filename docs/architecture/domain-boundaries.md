...
| Accounts | `/dashboard`, `/transactions`, `src/domains/accounts/public.ts`, wallet store and server readers | ETB wallet/transactions plus the separately owned USDT ledger | dashboard and wallet browser state is scoped to the exact authenticated user and token generation; accounting writes remain server-owned |
