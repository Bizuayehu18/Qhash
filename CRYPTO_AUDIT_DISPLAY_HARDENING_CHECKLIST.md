...
This document defines the small audit-display hardening items that should be completed before confirmation writes or crediting rely on the admin audit screen for reconciliation. It does not add code, database migrations, RPCs, UI actions, confirmation writes, wallet crediting, balance changes, transaction inserts, sweeping, signing, key handling, user-facing address exposure, or `crypto_auto_credit_enabled` changes.
...
Before the audit panel is used as part of confirmation or crediting operations, it should make display limitations explicit and preserve exact crypto amount semantics.
...
Before confirmation or crediting, admins and future code must still rely on canonical-chain revalidation:
...
Audit display values are useful for review, but canonical receipt/log verification remains the source of truth before confirmation and crediting.
...
1. Add UI-only display-cap warning for the admin audit panel.
2. Add exact numeric text source for audit rows, preferably through a read-only SQL view or RPC.
3. Update the audit helper to reject or count non-exact amount values rather than coercing them through JavaScript `number`.
4. Add admin audit counters for malformed or skipped audit rows.
5. Only then proceed to an admin-only BSC confirmation dry-run helper with canonical receipt/log revalidation.
